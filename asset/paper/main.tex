...
After LLaVA repair, the same Gemini/GPT validation prompt in Appendix~\ref{app:api-validation-prompt} is reused to verify repaired pairs. In the reported two-shot experiment, repaired-pair verification uses GPT-4.1-mini with the two-shot calibration block from Appendix~\ref{app:two-shot-validation-prompt}. The repaired answer is placed in the chosen-response field, the original hallucinated answer remains the rejected response, and the judge returns the same JSON schema:
\begin{quote}
\small
